...
class CatalogTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->cachePath = dirname(__DIR__)
            . DIRECTORY_SEPARATOR
            . 'tmp'
            . DIRECTORY_SEPARATOR
            . 'cache';
    ...
    {
        $dir = __DIR__ . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR;
        $catalog = $this->newCatalog([$dir . 'foo']);
        $this->assertOutput('foo', $catalog->getCompiled('test'));
        $catalog = $this->newCatalog([$dir . 'bar', $dir . 'foo']);
        $this->assertOutput('bar', $catalog->getCompiled('test'));
        $catalog = $this->newCatalog([$dir . 'baz', $dir . 'bar', $dir . 'foo']);
